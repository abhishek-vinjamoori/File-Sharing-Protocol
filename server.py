...
class Server:
    ...
    def __init__(self):
        ...
        self.alive = True
        self.availports=[51216, 51217, 51218, 51219, 51315]
        ...
        while self.alive:
            ...
            if spdata[0] == "IndexGet":
                if spdata[1] == "shortlist":
                    ...
                    info=""
                    try:
                        reprog=re.compile(' '.join(spdata[2:]).strip())
                        for f in filelist:
                            stat = os.stat(f)
                            size = str(stat.st_size)
                            mod_t = time.ctime(stat.st_mtime)
                ...
                if spdata[1]=="TCP":
                    ...
                    try:
                        ...
                        transfersock.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
                        transfersock.bind((self.hostname, transferport))
                        conn.send(("PASS "+str(transferport)).encode())
                        conn.recv(1)
                        conn.send((filename+' '+size+' '+mod_t+' '+hashval+'\n').encode())
                        data, (taddr, destinport) = transfersock.recvfrom(1024)
                        print("taddr destin",taddr,destinport)
                        with open(filename, "rb") as f:
                            for chunk in iter(lambda: f.read(4096), b""):
                                #print(chunk)
                                transfersock.sendto(chunk, (taddr, destinport))
                        #print("File sent!")
                        transfersock.close()
                        self.availports.append(transferport)
                    except IndexError:
